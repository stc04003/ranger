...
public:
  ...
  void initCpp(std::string dependent_variable_name,
  ...
  // 29 variables
  void initR(std::unique_ptr<Data> input_data,
	     uint mtry,
	     uint num_trees,
	     std::ostream* verbose_out,
	     uint seed,
  ...
  std::vector<std::vector<size_t>> getOobSampleIDs() {
    std::vector<std::vector<size_t>> result;
    for (auto& tree : trees) {
      result.push_back(tree->getOobSampleIDs());
    }
    return result;
  }

  std::vector<std::vector<size_t>> getSampleIDs() {
    std::vector<std::vector<size_t>> result;
    for (auto& tree : trees) {
      result.push_back(tree->getSampleIDs());
    }
    return result;
  }
